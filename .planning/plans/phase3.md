...
### 3.4 Settings Panel & UX
- [ ] 메인 뷰포트 UI: '투자 단위(매일/매주/매월)' 선택 및 해당 단위의 '투자 금액' 입력 폼만 노출하여 미니멀리즘 유지
- [ ] 설정 슬라이드/모달: 기존 원/달러 전환 기능을 유지하면서, 표기 옵션(실질/명목 변환) 및 환경 변수(기준금리, 환율) 입력창을 별도 패널로 분리
